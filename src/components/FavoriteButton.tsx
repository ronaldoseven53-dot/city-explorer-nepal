...
export default function FavoriteButton({ destinationId }: { destinationId: string }) {
  ...
  useEffect(() => {
    supabase.auth.getUser().then(({ data: { user } }) => {
      if (!user) return;
      setUserId(user.id);
      supabase.from("favorites").select("id").eq("user_id", user.id).eq("destination_id", destinationId).maybeSingle()
  ...
  const toggle = () => {
    ...
    start(async () => {
      if (saved) {
        await supabase.from("favorites").delete().eq("user_id", userId).eq("destination_id", destinationId);
        setSaved(false);
      } else {
